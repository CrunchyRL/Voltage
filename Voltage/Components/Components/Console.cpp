...
void ConsoleComponent::Write(int integer)
{
	Write(std::to_string(integer), TextColors::BrightWhite);
}
...
void ConsoleComponent::Initialize(const std::string& directory, const std::string& fileName)
{
	...
	SetConsoleTitle(L"Voltage Console (IF YOU CLOSE THE CONSOLE IT ALSO CLOSES ROCKET LEAGUE)");
}
